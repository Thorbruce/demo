...
@section('content')
    ...
    <div class="crumb_warp">
        ...
        <i class="fa fa-home"></i> <a href="{{URL('info')}}">首页</a> &raquo; <a href="{{url('category')}}">分类管理</a> &raquo; 添加分类
    </div>
    <!--面包屑导航 结束-->
            ...
            <table class="add_tab">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" class="cate_name" name="cate_name" >
    ...
    $(function(){
        $('#submit').click(function(){
          ...
          /*  $.post(url,{'pid':pid,'name':name,'titel':titel,'order':order,'keywords':keywords,'description':description,'_token':'{{csrf_field()}}'}, function (data) {
            ...
            */
            $.post(url,{'cate_pid':pid,'cate_name':name,'cate_title':title,'cate_order':order,'cate_keywords':keywords,'cate_description':description,'_token':'{{csrf_token()}}'}, function (data) {
                    if(data=='1'){
                        alert('增添分类成功');
